...
// A map between players and their symbols used on the board.
var pSymbolMap = {human:'X', machine:'O'};

/**
 * @desc Utility: Generate a random integer number for a given range.
 */
...
exports.getBoardStateSample = function(btype) {
  ...
  var playerBefore = '';
  board.data = x;
  board.playerBefore = playerBefore;
  return board;
}
...
/**
 * @desc Defines a function that takes a board state as input and determines
...
/**
 * @desc First step of the perfect player strategy.
 */
...
/**
 * @desc Second step of the perfect player strategy.
 */
exports.getMoveListBlock = function(board) {
  ...
  for (i=0; i<cellLocList.length; i++) {
    ...
    if ((x[ma[0]][ma[1]] == 'X') && (x[mb[0]][mb[1]] == 'X') && (x[mc[0]][mc[1]] == blank)) {
      ...
      moveList.push(ma);
    } else if ((x[ma[0]][ma[1]] == 'X') && (x[mb[0]][mb[1]] == blank) && (x[mc[0]][mc[1]] == 'X')) {
      moveList.push(mb);
    }
  }
  return moveList;
...
/**
 * @desc Third step of the perfect player strategy.
 */
...
/**
* @desc For a given board state, return all available moves.
*/
...
/**
* @desc Returns a next availale move.
*/
...
/**
 * @desc Given a current board state, update it using nextMove. Return the updated board. Note: You cannot update a cell that already as a value. If this happens, the function should throw an error.
 */
