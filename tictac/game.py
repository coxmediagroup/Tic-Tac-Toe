...
def get_next_gamestate(game, player=None):
    ...
    #on our next turn
    opponent_movetree = { m : generate_movetree(game.apply_move(m)) for m, gs in movetree.iteritems() }
    ...
    for m, mt in opponent_movetree.iteritems():
        for om, ogs in mt.iteritems():
            print om, ogs
            print get_next_gamestate(ogs)
    ...
    for m, mt in opponent_movetree.iteritems():
        print "Opps: ",m, mt
        ournext_movetree = { 
                mx : generate_movetree(gx.apply_move(mx))
                for mx, gx 
                in mt.iteritems() 
        }
        for m2, gs2 in ournext_movetree:
            print gs2
            pp.pprint(mt2)
    """
