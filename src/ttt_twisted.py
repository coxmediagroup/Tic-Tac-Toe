...
class TelnetGame(Game):
    ...
    def run(self):
        ...
        if Storage(self.protocol.identifier)._game_board.place(self.active_player.shape,next_move):
            self.move_count += 1
            self.turnComplete()
        if self.running:
    ...
    def checkGameOver(self):
        board, v, nw, sw = Storage(self.protocol.identifier)._game_board.winLists()
        ...
        if Storage(self.protocol.identifier)._game_instance.active_player == self:
            ...
            if move :
                for row in self.keymap:
                    if move in row:
                        self.d.callback((self.keymap.index(row), row.index(move)))
                        self.d = defer.Deferred()
            else:
                self.turn()
        else:
...
class TelnetAi(Ai):
    ...
    def turnComplete(self, *args):
        ...
        self.displayMsg(Storage(self.protocol.identifier)._game_board.drawBoard())
