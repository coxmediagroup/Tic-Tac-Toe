...
class Engine(object):
    ...
    def next_move(self, board, player):
        ...
        Returns:
            A tuple (x, y) representing the best _move from the current
    ...
    def get_state(self, board):
        ...
        Returns:
            The state (as defined in game.Game) of the game for the given
            board.
        '''
    ...
    def get_legal_moves(self, board):
        ...
        Returns:
            A list of tuples (x, y) representing valid _move for the given
            board (game state).
        '''
...
class NegamaxEngine(Engine):
    ...
    def _negamax(self, board, depth, player):
        ...
        for move in self.get_legal_moves(board):
            ...
            x = -self._negamax(next_board, depth + 1,
...
class RulesBasedEngine(Engine):
    ''' Tic-Tac-Toe game engine that uses rules to calculate
        ...
        Attributes:
            _strategies: An ordered list of methods which represent different
                         game strategies to use.

        Class attributes:
            _CORNERS: A list of corner coordinates given as tuples (x, y)
            _OPPOSITE_CORNERS: A map of opposite corner coordinates.
            _SIDES: A list of coordinates for side fields of the board.
    '''
    ...
    def _play_win(self, board, player, total, scores):
        ...
        for win_value in self._WIN_VALUES:
            value = win_value - (scores[player] & win_value)
            move = numpy.where(self._COMP_MATRIX == value)
    ...
    def _play_block_fork(self, board, player, total, scores):
        ...
        for move in self.get_legal_moves(board):
            ...
            winning_moves = self._play_win(board, player, new_total,
                                           new_scores)
            # Get fork moves for the opponent on the new board
            fork_moves = self._play_fork(board, opponent, new_total,
                                         new_scores)
            # The move is valid only if it does not create a fork
            for winning_move in winning_moves:
                if not winning_move in fork_moves:
                    moves.append(move)
            # Revert the board
    ...
    def _play_opposite_corner(self, board, player, total, scores):
        ...
        for corner in self._CORNERS:
            if board[corner[0], corner[1]] == opponent:
                ...
                if board[opp_corner[0], opp_corner[1]] == EMPTY:
                    moves.append(opp_corner)
